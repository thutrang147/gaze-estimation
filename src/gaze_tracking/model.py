...
import math
try:
    # OpenVINO 2025+ (new API)
    from openvino import Core
except ImportError:
    # OpenVINO 2024 and older (deprecated API)
...
class Model(metaclass=abc.ABCMeta):
    ...
    def _init_input_output(self, model):
        self.input_name = model.input(0).get_any_name()
        self.input_shape = model.input(0).shape
        self.output_name = model.output(0).get_any_name()
        self.output_shape = model.output(0).shape
    ...
    def get_input_shape(self, input_name=None):
        ...
        for inp in self.model.inputs:
            if inp.get_any_name() == input_name:
                return inp.shape
        return None
    ...
    def exec_net(self, request_id, inputs):
        if isinstance(inputs, dict):
            ...
            self.infer_request.infer({self.input_name: inputs})
    ...
    def get_outputs(self, request_id):
        return self.infer_request.results

    def get_output(self, request_id):
        return self.infer_request.get_tensor(self.output_name).data
...
class HeadPoseEstimation(Model):
    ...
    def load_model(self):
        super().load_model()
        # Get all output names for multi-output model
        for output in self.model.outputs:
            self.output_names[output.get_any_name()] = output
    ...
    def predict(self, image):
        ...
        if status == 0:
            ...
            for name in self.output_names:
                outputs[name] = self.infer_request.get_tensor(name).data
...
class GazeEstimation(Model):
    ...
    def predict(self, right_eye_image, head_pose_angles, left_eye_image):
        ...
        right_eye_image, head_pose_angles, left_eye_image = self._preprocess_gaze_input(
...
class OpenClosedEye(Model):
    ...
    def predict(self, right_eye_image, left_eye_image):
        # right eye
        right_eye_image_input = self._preprocess_input(right_eye_image)
        self.exec_net(0, right_eye_image_input)
        status = self.wait(0)
        if status == 0:
            outputs = self.get_output(0)
            out_right = self._preprocess_output(outputs)
        # left eye
        left_eye_image_input = self._preprocess_input(left_eye_image)
        self.exec_net(0, left_eye_image_input)
        status = self.wait(0)
        if status == 0:
            outputs = self.get_output(0)
    ...
    def _preprocess_output(self, outputs):
        return self.out_val[np.argmax(np.reshape(outputs, (1, 2)))]
...
class Pupils(Model):
    ...
    def _preprocess_output(self, outputs):
        ...
        label_img = skimage.measure.label(binarized)
...
class EyeModel:
    ...
    def __init__(self, directory,
                 subdir_face=os.path.join("intel", "face-detection-adas-0001", "FP32", "face-detection-adas-0001"),
                 subdir_landmark=os.path.join("intel", "landmarks-regression-retail-0009", "FP32",
                                              "landmarks-regression-retail-0009"),
                 subdir_headpose=os.path.join("intel", "head-pose-estimation-adas-0001", "FP32",
                                              "head-pose-estimation-adas-0001"),
                 subdir_gaze=os.path.join("intel", "gaze-estimation-adas-0002", "FP32", "gaze-estimation-adas-0002"),
                 subdir_open_close=os.path.join("intel", "open-closed-eye-0001", "FP32", "open-closed-eye-0001"),
                 subdir_pupil=os.path.join("intel", "PupilSegmentation", "pupils_segmentation"),
                 subdir_landmark_35=os.path.join("intel", "facial-landmarks-35-adas-0002", "FP32",
                                                 "facial-landmarks-35-adas-0002")) -> None:
    ...
    def get_gaze(self, frame, imshow=False):
        ...
        for face_box in face_boxes:
            ...
            if imshow:
                cv2.putText(frame, f"RightEye {out_right}; LeftEye {out_left}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
    ...
    def get_iris_Cnn(self, frame):
        try:
            ...
            for eyes in eye_pairs:
                ...
                right_image = np.hstack((cv2.resize(cv2.cvtColor(eyes[0], cv2.COLOR_BGR2GRAY), (200, 200)),
                                         cv2.resize(rightprediction * 255.0, (200, 200)),
                                         cv2.resize(rightmorphedMask, (200, 200))))
                # left eye
                ...
                left_image = np.hstack(
                    (cv2.resize(gray_image, (200, 200)), cv2.resize(leftprediction * 255.0, (200, 200)),
                     cv2.resize(leftmorphedMask, (200, 200))))
    ...
    def get_iris_CnnHoughCircles(self, frame):
        ...
        try:
            ...
            for eyes in eye_pairs:
                ...
                image = np.hstack(
                    (right_image, cv2.resize(left_image, np.shape(right_image)[1::-1], interpolation=cv2.INTER_AREA)))
                img = np.hstack((rightprediction, cv2.resize(leftprediction, np.shape(rightprediction)[1::-1],
    ...
    def get_iris_HoughCircles(self, frame):
        ...
        try:
            ...
            for eyes in eye_pairs:
                ...
                image = np.hstack(
                    (right_eye, cv2.resize(left_eye, np.shape(right_eye)[1::-1], interpolation=cv2.INTER_AREA)))
                img = np.hstack(
                    (right_img, cv2.resize(left_img, np.shape(right_img)[1::-1], interpolation=cv2.INTER_AREA)))
                img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
                image = np.vstack((image, img))
                # image = right_eye
    ...
    def _detect_pupil(self, frame, eye_img):
        ...
        circles = cv2.HoughCircles(eye_img, cv2.HOUGH_GRADIENT, 1, 20, param1=50, param2=12, minRadius=0,
                                   maxRadius=int(np.shape(frame)[0] / 2))  # return (x,y,radius)
